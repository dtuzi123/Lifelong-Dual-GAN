...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        newX1,newX2 = Exchange_Dataset(self.mnist_train_x,self.mnist_label,self.svhn_train_x,self.svhn_label)
        self.mnist_train_x = newX1
        self.svhn_train_x = newX2

        newX3,newX4 = Exchange_Dataset(self.cifarTrainX,self.cifarTrainLabels,self.FashionTrain_x,self.FashionTrain_label)
        self.cifarTrainX = newX3
        self.FashionTrain_x = newX4
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.saver.restore(sess, 'models/TwinGenerative_Unsupervised_TaskBounders2')
            ...
            ims("results/" + "Proposed_VAE_recon" + str(0) + ".png", merge(reconstruction[:64], [8, 8]))
            ims("results/" + "Proposed_Real" + str(0) + ".png", merge(batch_images[:64], [8, 8]))

            batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
            g1, g2 = self.sess.run([self.GAN_gen1, self.GAN_gen2], feed_dict={self.z: batch_z})
            ims("results/" + "GAN1_1" + str(0) + ".png", merge(g1[:64], [8, 8]))
            ims("results/" + "GAN2_2" + str(0) + ".png", merge(g2[:64], [8, 8]))
            ...
            IFashionError = self.Calculate_ReconstructionErrors(self.cifarTestX)
            ...
            IFashionElbo = self.Calculate_Elbo(self.cifarTestX)

            averageElbo = mnistElbo + fashionElbo + svhnElbo+IFashionElbo
            averageReco = mnistError + fashionError + svhnError+IFashionError
            averageElbo = averageElbo / 4.0
            averageReco = averageReco / 4.0
            print(mnistError)
            print('\n')
            print(mnistElbo)
            print('\n')
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for taskIndex in range(taskCount):
                ...
                if taskIndex == 0:
                    currentTrainX = self.cifarTrainX
                elif taskIndex == 1:
            ...
            self.saver.save(self.sess, "models/TwinGenerative_Unsupervised_TaskBounders2")
