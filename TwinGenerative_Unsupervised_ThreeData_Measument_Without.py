...
class LifeLone_MNIST(object):
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
                ...
                for epoch in range(start_epoch, self.epoch):
                    ...
                    for idx in range(start_batch_id, self.num_batches*t):
                        ...
                        if taskIndex == 0:
                            ...
                            _, d_loss2, = self.sess.run([self.d_optim2, self.d_loss2],
                                                        feed_dict={self.inputs: batch_images,
                                                                   self.z: batch_z, self.gan_inputs: batch_images})

                            # update G and Q network
                            _, g_loss2 = self.sess.run(
                                [self.g_optim2, self.g_loss2],
                                feed_dict={self.inputs: batch_images, self.z: batch_z,self.gan_inputs:batch_images})
                        else:
                            generatedSamples = self.sess.run(self.GAN_gen1,
                                                             feed_dict={self.inputs: dataX[0:self.batch_size],
                                                                        self.z: batch_z})
            ...
            f = open("results/MNIST_reco_Without" + str(beta) + ".txt", "w", encoding="utf-8")
            ...
            f = open("results/MNIST_elbo_Without" + str(beta) + ".txt", "w", encoding="utf-8")
            for i in range(np.shape(lossArr1)[0]):
